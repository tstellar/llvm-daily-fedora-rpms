...
if [ -d "/opt/llvm/" -a $(arch) = "x86_64" ]; then
  cxx_compiler=/opt/llvm/bin/clang++
  c_compiler=/opt/llvm/bin/clang
else
  cxx_compiler=/usr/bin/clang++
  c_compiler=/usr/bin/clang
fi

cmake -G Ninja -B build -S llvm -DCMAKE_BUILD_TYPE=Release -DLLVM_ENABLE_PROJECTS=clang -DLLVM_TARGETS_TO_BUILD=Native -DLLVM_BINUTILS_INCDIR=/usr/include/ -DCMAKE_CXX_COMPILER_LAUNCHER=ccache -DCMAKE_CXX_COMPILER=$cxx_compiler -DCMAKE_C_COMPILER=$c_compiler
