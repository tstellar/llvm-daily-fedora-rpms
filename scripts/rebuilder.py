...
def get_tier1_pkgs(version : str) -> set[str]:
    base = dnf.Base()
    conf = base.conf
...
def pkg_is_ftbfs(ftbfs_data, pkg: str, tag: str = "f44"):
...
def main() -> None:
    ...
    if args.command == "rebuild":
        ...
        target_chroots = centos_stream10_chroots + centos_stream9_chroots + fedora_chroots
        try:
            copr_client.project_proxy.get(project_owner, project_name)
        except:
            create_new_project(project_owner, project_name, copr_client, target_chroots, additional_packages = None, with_opts = None)
            # Set repo priority so that built packages that depend on a specific
            # LLVM snapshot version do not get installed.
            copr_client.project_proxy.edit(project_owner, project_name,  repo_priority=1000)
        ...
        copr_client.project_proxy.edit(
            ...
            project_name,
            additional_repos=[
                f"copr://@fedora-llvm-team/llvm-compat-packages",
            ],
        )
        ...
        for chroot in list(target_chroots):
            ...
            repos = []
            for r in copr_client.project_chroot_proxy.get(project_owner, project_name, chroot)['additional_repos']:
                if args.skip_same_version and r == snapshot_url:
                    print(f"Not building for {chroot} since snapshot version is the same as the last build")
                    target_chroots.remove(chroot)
                if not r.startswith("copr://@fedora-llvm-team/llvm-snapshots-big-merge"):
                    repos.append(r)
        ...
        centos_stream9_chroots = [c for c in centos_stream9_chroots if c in target_chroots]
        for pkg in centos9_pkgs:
            build_pkg(project_owner, project_name, copr_client, pkg,
        ...
        centos_stream10_chroots = [c for c in centos_stream10_chroots if c in target_chroots]
        ...
        with urllib.request.urlopen("https://koschei.fedoraproject.org/api/v1/packages") as url:
